...
##### **14.3.1.1 Exercises**
1. A backslash is used to escape special behavior  (like a symbol), so none of the string match
2. \"'\\\\
3. - 
...
You can specify # of matches precisely
* {n}: exactly n
* {n,}: n or more
* {,m}: at most m
* {n,m}: between n and m
...
##### **14.3.4.1 Exercises**
1. Describe equivalents of ?, +, * in {m,n} form
* {,1} at most 1, 0 or 1 
* {1,} 1 or more
* {0,} 0 or more 
...
* (.)\1\1 : a character appearing 3x in a row, ex: aaa
* "(.)(.)\\2\\1" : two characters repeated, but second pair is reversed, ex: abba
* (..)\1 : two characters repeated, ex: abab
* "(.).\\1.\\1" : original character -> any chara -> original -> any -> original, ex: azaxa
* "(.)(.)(.).*\\3\\2\\1" : three characters followed by 0 or more characters (any) followed by the same but in reverse order, ex: abc0cba
...
* str_replace() and str_replace_all() allow you to replace matches w/ new strings
...
#### **14.4.7 stringi**
* stringi is designed to be comprehensive
* contains all functions you may ever need (250)
* stringi package is similar to stringr
