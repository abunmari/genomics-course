...
```{r}
data(vcfR_example)  #loads dataset
vcf
```
### The meta region

* contains info about the file, its creation, and info to interpret abbreviations used elsewhere in the file
...
* When an element parameter is included, only information about the element is returned
...
```{r}
vcf <- read.vcfR("data/pinfsc50_filtered.vcf")
```
...
* after  manipulating the file, we can save as a VCF file w/ write.vcf()
...
1) How would you find more information about read.vcfR()? 
...
```{r, include = FALSE}
library('vcfR')
vcf <- read.vcfR("data/myVCFdata_filtered.vcf.gz")
```
...
plot(chrom)
```
* second plot, called chromo plot, displays the same information as the plot method only it distributes the data along its chromosomal coordinates. 
*includes a representation of the annotation data 
...
* 1) summaries are made on a per variant basis (sample size, allele counts, heterozygosity, and effective size)
* 2) summaries made on a per window basis
...
* summarize, take averages of the data
